...
temp = 0
os.chdir(curDir)
with open(curDir + '/done_down_zfmt.txt', 'a+') as fileObj:
    read_lines = fileObj.readlines()

for i in range(39, 696):
    print('第' + str(i) + '页')
    url = "https://www.790nd.com/piclist5/index_" + str(i) + ".html"
    if i == 1:
    ...
    for j in range(0, len(itemUrl)):
        ...
        if file_url.startswith("http"):
            ...
            if url_num not in read_lines:
                with open(curDir + '/done_down_zfmt.txt', 'a+') as downObj:
