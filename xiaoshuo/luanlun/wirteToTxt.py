...
for i in range(1,44):
    print('第'+str(i)+'页')
    url="https://www.4455sk.com/xiaoshuo/list-家庭乱伦-"+str(i)+".html"
    print(url)
    ...
    for j in range(0,len(itemUrl)):
        ...
        if(fileUrl!=None and fileUrl.find('.html')>=0):
            ...
            print("fileUrl:"+fileUrl)
            os.chdir(path)
            f=open('xiaoshuo_luanlun_'+datetime.datetime.now().strftime('%Y-%m-%d')+'_'+str(temp//500)+'.txt','a+')
            f.write(fileUrl+'\n')
            f.close()
print("打印完成")
