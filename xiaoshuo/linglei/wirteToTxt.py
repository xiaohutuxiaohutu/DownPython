...
temp=0
path='D:/down/xiaoshuo/linglei/'
if not(os.path.exists(path)):
    os.makedirs(path)
for i in range(1,45):
    print('第'+str(i)+'页')
    url="https://www.4455sk.com/xiaoshuo/list-另类小说-"+str(i)+".html"
    ...
    for j in range(0,len(itemUrl)):
        ...
        if(fileUrl!=None and fileUrl.find('.html')>=0):
            ...
            print("fileUrl:"+fileUrl)
            os.chdir(path)
            f=open('linglei_'+datetime.datetime.now().strftime('%Y-%m-%d')+'_'+str(temp//500)+'.txt','a+')
            f.write(fileUrl+'\n')
            f.close()
print("打印完成")
