...
for i in range(1,45):
    print('第'+str(i)+'页')
    url="https://www.4455sk.com/xiaoshuo/list-校园春色-"+str(i)+".html"
    ...
    itemUrl=soup.select("body div[class='maomi-content'] main[id='main-container'] div[class='text-list-html'] div ul li a")
    ...
    for j in range(0,len(itemUrl)):
        ...
        if(fileUrl!=None and fileUrl.find('.html')>=0):
            ...
            print("fileUrl:"+fileUrl)
            os.chdir(path)
            f=open('xiaoyuan_'+datetime.datetime.now().strftime('%Y-%m-%d')+'_'+str(temp//500)+'.txt','a+')
            f.write(fileUrl+'\n')
            f.close()
print("打印完成")
