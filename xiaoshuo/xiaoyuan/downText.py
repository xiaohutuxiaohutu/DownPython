...
file=open("D:/down/xiaoshuo/xiaoyuan/xiaoyuan_2019-01-27_1.txt")
ip_list=get_ip_list(proxyipurl)
#获取总行数
path='D:/down/xiaoshuo/xiaoyuan/'+datetime.datetime.now().strftime('%Y-%m-%d')+'/'
...
for num,value in enumerate(file,1):
    ...
    if len(textContent)==0:
        #os.chdir(path)
        f=open('xiaoyuan_'+datetime.datetime.now().strftime('%Y-%m-%d')+'_未下载.txt','a+')
